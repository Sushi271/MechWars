...
{
    public class HarvestOrder_OLD : Order_OLD
    {
        MoveOrder_OLD move;
        CollectOrder_OLD collect;
        DepositOrder_OLD deposit;
        ...
        public HarvestOrder_OLD(Unit orderedUnit, Resource resource)
        ...
        public HarvestOrder_OLD(Unit orderedUnit, Building refinery)
            : base("Harvest", orderedUnit)
        {
        ...
        void ExecuteMode()
        {
            if (mode == HarvestMode.Collect)
            {
                ...
                else
                {
                    ...
                    {
                        if (collect == null)
                            collect = new CollectOrder_OLD(Unit, Resource);
                        if (!collect.InRange)
                        {
                            if (move == null || reloadMove)
                            {
                                move = new MoveOrder_OLD(Unit, Resource.Coords.Round());
                                reloadMove = false;
                            }
                            move.Update();
                        }
            ...
            {
                if (deposit == null)
                    deposit = new DepositOrder_OLD(Unit, Refinery);
                ...
                else
                {
                    if (!deposit.InRange)
                    {
                        if (move == null || reloadMove)
                        {
                            move = new MoveOrder_OLD(Unit, Refinery.Coords.Round());
                            reloadMove = false;
                        }
                        move.Update();
                    }
