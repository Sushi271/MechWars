        ...
        void Update()
        {
            ...
            foreach (var b in buttons)
            {
                ...
                if (active)
                {
                    ...
                    if (b.orderAction is BuildingConstructionOrderAction)
                    {
                        var coa = (BuildingConstructionOrderAction)b.orderAction;
                        active = coa.CheckRequirements(Globals.HumanArmy);
                    }
                    else if (b.orderAction is UnitProductionOrderAction)
                    {
                        var poa = (UnitProductionOrderAction)b.orderAction;
                        active = poa.CheckRequirements(Globals.HumanArmy);
                    }
                    else if (b.orderAction is TechnologyDevelopmentOrderAction)
                    {
                        var doa = (TechnologyDevelopmentOrderAction)b.orderAction;
